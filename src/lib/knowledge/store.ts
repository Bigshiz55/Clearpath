...
/** Everything the knowledge layer holds about ONE title — the header plus all
 *  compiled subject facts. Founder inspection + honest degradation: `absent`
 *  distinguishes "tables unreadable/missing" from "compiled: nothing". */
export async function readTitleKnowledge(
  tmdbId: number,
  mediaType: MediaType,
): Promise<{
  header: Record<string, unknown> | null;
  facts: StoredSubjectFact[];
  absent: boolean;
}> {
  try {
    const admin = createAdminClient();
    const [h, f] = await Promise.all([
      admin.from('title_knowledge').select('*').eq('tmdb_id', tmdbId).eq('media_type', mediaType).maybeSingle(),
      admin
        .from('title_subject_facts')
        .select('tmdb_id, media_type, subject, centrality, confidence, source_count, disputed, decided_by, evidence, compiler_version, compiled_at')
        .eq('tmdb_id', tmdbId)
    ...
    if (h.error && f.error) return { header: null, facts: [], absent: true };
    const facts = ((f.data ?? []) as Array<Record<string, unknown>>).map((row) => ({
      tmdbId,
      mediaType,
      subject: String(row.subject),
      centrality: String(row.centrality) as FactCentrality,
    ...
    return { header: (h.data as Record<string, unknown> | null) ?? null, facts, absent: false };
  } catch {
    return { header: null, facts: [], absent: true };
  }
}
