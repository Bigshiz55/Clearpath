...
import { getPackPreview } from './packs';
import type { Pack } from './types';

/**
 * RELIABILITY SPRINT — item 5, TV date/time logic. `pack_premiere_calendar`
 * rows carry `premiere_date`, a calendar DAY (the UTC date of the earliest
 * premiere airing), NOT an instant. A premiere that already aired earlier
 * today still has today's date, so picking the query's first row by date
 * order alone can surface something that's already over. `getPackPreview`
 * must pick the soonest premiere whose real `start_at_utc` instant is still
 * in the future.
 */
