...
/* KEYED OVER THE CANONICAL SET so a rich axis can be phrased at all. Falling
   ...
   `DIMENSIONS` is deliberately not consulted here any more: `TASTE_AXES` carries
   the same fifteen entries under the same keys with the same `low`/`high` copy
   (they are flagged `legacy: true` there), so this is a superset rather than a
   substitution, and no existing phrase changes. */
