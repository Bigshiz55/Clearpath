...
/**
 * THE ROUTE FOLDS THE STATEMENT INTO THE CONVERSATION — structurally pinned,
 * because this is exactly the wiring that "Noted" depends on to be true.
 */
import { readFileSync } from 'node:fs';

describe('the statement branch folds taste into the conversation state', () => {
  const route = readFileSync('src/app/api/ask/route.ts', 'utf8');

  it('absorbStatement runs inside the isBareStatement branch, before the reply', () => {
    const branch = route.slice(route.indexOf('isBareStatement(canonical) && !criticRequest'));
    const absorbAt = branch.indexOf('absorbStatement(convState, canonical');
    const replyAt = branch.indexOf('acknowledgeStatement(canonical)');
    expect(absorbAt, 'the fold is missing from the statement branch').toBeGreaterThan(-1);
    expect(absorbAt, 'the fold must happen before the acknowledgement is sent').toBeLessThan(replyAt);
  });

  it('the fold resolves genres through the canonical vocabulary boundary', () => {
    expect(route).toMatch(/absorbStatement\(convState, canonical, genreIdFor\)/);
  });

  it('the fold’s notes reach the user — a carried constraint is announced', () => {
    expect(route).toMatch(/convInterpretation = \[\.\.\.convInterpretation, \.\.\.absorbed\.notes\]/);
  });
});
