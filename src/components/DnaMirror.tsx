  ...
  if (failed) {
    return (
      <div className="mt-8 card p-8 text-center">
        ...
        <h2 className="mt-3 text-xl font-bold text-white">{t('ask.dnaBuilding')}</h2>
        <Link href="/app" className="btn-primary mt-5 inline-flex">{t('ask.seeMyRecs')}</Link>
      </div>
    );
  }
  if (!m) {
    return (
      <div className="mt-8 flex flex-col items-center gap-3 text-slate-400">
        ...
        <span className="text-sm">{t('ask.readingTaste')}</span>
      </div>
    );
  }
    ...
    <div className="mt-6 space-y-5">
      ...
      <section className="card overflow-hidden p-0">
        <div className="bg-gradient-to-br from-brand-500/20 via-fuchsia-500/10 to-transparent p-5 sm:p-6">
          <div className="text-xs font-bold uppercase tracking-[0.15em] text-brand-300">{t('ask.whatWeRead')}</div>
          ...
          <div className="mt-3 text-xs font-semibold">
            {m.ready ? (
              <span className="text-emerald-200">{t('ask.strongRead', { pct })}</span>
            ) : (
              <span className="text-amber-200">{t('ask.stillKnowing')}</span>
            )}
          </div>
        </div>
      </section>
      ...
      {m.picks.length > 0 && (
        <section>
          <h3 className="mb-2 text-sm font-bold text-white">{m.ready ? t('ask.picksFollow') : t('ask.startingPoints')}</h3>
      ...
      <section className="card p-4 text-center">
        {vote == null ? (
          <>
            <div className="text-sm font-bold text-white">{t('ask.feelLikeYou')}</div>
            <div className="mt-3 flex items-center justify-center gap-2">
              <button onClick={() => say('yes')} className="rounded-full border border-emerald-400/50 bg-emerald-500/15 px-4 py-2 text-sm font-bold text-emerald-100 transition hover:bg-emerald-500/25">{t('ask.nailedIt')}</button>
              <button onClick={() => say('not_quite')} className="rounded-full border border-white/20 bg-white/5 px-4 py-2 text-sm font-bold text-slate-200 transition hover:bg-white/10">{t('ask.notQuite')}</button>
            </div>
          </>
        ) : vote === 'yes' ? (
          <div className="text-sm font-semibold text-emerald-200">{t('ask.loveItSharper')}</div>
        ) : (
          <div className="text-sm text-slate-200">
            {t('ask.notQuitePrefix')}
            <Link href="/app/dna" className="font-bold text-brand-300 underline">{t('ask.watchDnaPageLink')}</Link>{t('ask.notQuiteSuffix')}
          </div>
        )}
      </section>

      <div className="flex flex-wrap justify-center gap-3">
        {onReplay && <button onClick={onReplay} className="btn-secondary">{t('ask.rateAnotherRound')}</button>}
        <Link href="/app/watch" className="btn-primary">{t('ask.seeAllRecs')}</Link>
      </div>
    </div>
  );
}
