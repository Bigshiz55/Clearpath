...
const MOODS = [
  { key: 'any', emoji: '🎬' }, { key: 'light', emoji: '🌤️' },
  { key: 'intense', emoji: '🔥' }, { key: 'funny', emoji: '😂' },
  { key: 'cinematic', emoji: '🎥' }, { key: 'short', emoji: '⏱️' },
];
...
export function LiveCourt({ code }: { code: string }) {
  ...
  async function invite() {
    ...
    if (share) {
      try { await share({ title: t('together.shareTitle'), text: t('together.shareText'), url: shareUrl }); return; } catch { /* cancelled */ }
    }
  ...
  async function refresh() {
    ...
    if (error) { if (error.code === '42P01') setErr(t('together.liveCourtNotSetup')); return; }
  ...
  async function startCourt() {
    ...
    const d = await res.json();
    setStarting(false);
    if (!res.ok || d.error) setErr(d.error ?? t('together.couldNotStart'));
    else void refresh();
  }
  ...
  async function repick(action: 'veto' | 'more', index?: number) {
    ...
    const d = await res.json();
    setBusy(false);
    if (!res.ok || d.error) setErr(d.error ?? t('together.couldNotUpdate'));
    else void refresh();
  }

  if (notFound) {
    return <Shell><div className="card p-8 text-center"><div className="text-3xl">🔗</div><p className="mt-3 text-sm text-slate-400">{t('together.roomNotFound')}</p><Link href="/app" className="btn-secondary mt-4 inline-flex">{t('together.openWatchVerdict')} →</Link></div></Shell>;
  }
  if (!state) return <Shell><div className="text-sm text-slate-400">{t('together.connecting')}</div></Shell>;
    ...
    return (
      <Shell>
        <div className="card p-5">
          <div className="text-sm font-semibold text-white">{t('together.joinTheCourt')}</div>
          <p className="mt-1 text-xs text-slate-400">{t('together.joinIntroBefore')}<span className="font-semibold text-slate-200">{t('together.joinIntroYou')}</span>{t('together.joinIntroAfter')}</p>
          {mine?.signedIn && mine.name && (
            <button onClick={() => setName(mine.name!)} className="mt-3 w-full rounded-xl border border-brand-400/40 bg-brand-500/15 px-3 py-2 text-sm font-semibold text-brand-100">✨ {t('together.imName', { name: mine.name })}</button>
          )}
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder={t('together.yourName')} className="input mt-3" maxLength={40} />
          <div className="mt-3 text-xs font-semibold text-slate-300">{t('together.yourMood')}</div>
          <div className="mt-1 grid grid-cols-3 gap-2">
            {MOODS.map((m) => <button key={m.key} onClick={() => setMood(m.key)} className={`rounded-xl border py-2 text-xs font-semibold ${mood === m.key ? 'border-brand-400/50 bg-brand-500/20 text-brand-100' : 'border-white/15 bg-white/5 text-slate-300'}`}>{m.emoji} {t(`together.mood.${m.key}`)}</button>)}
          </div>
          {err && <p className="mt-3 text-xs text-red-300">{err}</p>}
          <button onClick={join} disabled={joining || !name.trim()} className="btn-primary mt-4 w-full py-3">{joining ? t('together.joining') : `${t('together.joinTheCourt')} →`}</button>
        </div>
      </Shell>
    );
  }
      ...
      <Shell>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between">
            <div className="text-sm font-semibold text-white">{t('together.whatWatch')}</div>
            <span className="text-xs text-slate-400">{picks.length}/8</span>
          </div>
          <p className="mt-1 text-xs text-slate-400">{t('together.wishlistIntro')}</p>

          <div className="relative mt-3">
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t('together.searchPlaceholder')} className="input" />
            {(hits.length > 0 || searching) && (
              <div className="absolute z-10 mt-1 max-h-72 w-full overflow-auto rounded-xl border border-white/10 bg-ink-900 shadow-xl">
                {searching && hits.length === 0 && <div className="p-3 text-xs text-slate-500">{t('together.searching')}</div>}
                {hits.map((h) => {
                  ...
                  return (
                    <button key={keyOf(h)} onClick={() => addPick(h)} disabled={already || picks.length >= 8} className="flex w-full items-center gap-3 border-b border-white/5 p-2 text-left hover:bg-white/5 disabled:opacity-40">
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="block text-[11px] uppercase tracking-wide text-slate-500">{h.mediaType === 'tv' ? t('together.tvSeries') : t('together.movie')}</span>
                      </span>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {picks.map((p) => (
                <span key={keyOf(p)} className="inline-flex items-center gap-1.5 rounded-full border border-brand-400/40 bg-brand-500/15 py-1 pl-3 pr-1.5 text-xs font-semibold text-brand-100">
                  {p.title}
                  <button onClick={() => removePick(keyOf(p))} aria-label={t('together.removeTitle', { title: p.title })} className="grid h-4 w-4 place-items-center rounded-full bg-white/15 text-[10px] leading-none hover:bg-white/30">×</button>
                </span>
              ))}
            </div>
          )}
        ...
        <div className="card mt-4 p-5 text-center">
          <div className="text-xs uppercase tracking-widest text-brand-300">{plural('together.waitingRoom', state.participants.length, {})}</div>
          ...
          <div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-3">
            <div className="text-xs font-semibold text-slate-300">{t('together.inviteOthers')}</div>
            <div className="mt-2 flex items-center justify-center gap-2">
              <button onClick={invite} className="btn-primary text-sm">📨 {t('together.sendInvite')}</button>
              <button onClick={showQr} className="btn-secondary text-sm">{qr ? t('together.hideQr') : t('together.qrCode')}</button>
            </div>
          ...
          {isHost ? (
            <button onClick={startCourt} disabled={starting || state.participants.length < 2 || !anyPicks} className="btn-primary mt-5 w-full py-3">
              {starting ? t('together.deliberating') : state.participants.length < 2 ? t('together.need2') : !anyPicks ? t('together.addTitlesFirst') : `⚖️ ${t('together.deliverRuling')}`}
            </button>
          ) : (
            <div className="mt-5 text-sm text-slate-400">{t('together.waitingHost')}</div>
          )}
        </div>
      </Shell>
    );
        ...
        <Shell>
          <div className="grid min-h-[60vh] place-items-center text-center">
            <div>
              ...
              <div className="wv-order mt-4 text-2xl font-black uppercase tracking-[0.2em] text-gold-300">{t('together.orderInCourt')}</div>
              <div className="mt-2 text-sm text-slate-400">{t('together.rankingPicks')}</div>
            </div>
          </div>
        </Shell>
      );
        ...
        <div className="wv-verdict-in">
          <div className="text-center">
            <div className="inline-flex items-center gap-1.5 rounded-full border border-gold-400/40 bg-gold-500/15 px-3 py-1 text-[11px] font-black uppercase tracking-[0.15em] text-gold-200">
              <span aria-hidden>🔨</span> {t('together.theRuling')}
            </div>
            <p className="mt-2 text-sm text-slate-300">{plural('together.rankedForAll', finalists[0]?.perMember.length ?? state.participants.length, {})}</p>
          </div>
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-2">
            <button onClick={() => repick('more')} disabled={busy} className="btn-secondary inline-flex justify-center disabled:opacity-50">🔄 {t('together.show3More')}</button>
            <Link href={`/app/title/${finalists[0]!.mediaType}/${finalists[0]!.id}`} className="btn-primary inline-flex justify-center">▶ {t('together.openNo1')} →</Link>
          </div>
          <div className="mt-3 text-center">
            <Link href="/app/together" className="text-xs font-semibold text-slate-400 hover:text-slate-200">{t('together.startNewRound')}</Link>
          </div>
        </div>
      </Shell>
    );
  ...
  return <Shell><div className="text-sm text-slate-400">{t('together.loading')}</div></Shell>;
}
      ...
      <div className="flex gap-3">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="mt-1 flex flex-wrap gap-1.5">
            <span className="rounded-full border border-brand-400/40 bg-brand-500/15 px-2 py-0.5 text-[11px] font-bold text-brand-100">{t('together.groupFit', { fit: f.fit })}</span>
            {f.attributes.slice(0, 2).map((a, j) => <span key={j} className="rounded-full bg-white/10 px-2 py-0.5 text-[11px] text-slate-200">{a}</span>)}
          </div>
          {f.pickedBy.length > 0 && (
            <div className="mt-1.5 text-[11px] text-emerald-200">⭐ {t('together.pickedBy', { names: f.pickedBy.join(', ') })}</div>
          )}
  ...
  return (
    <div className="min-h-dvh">
      <header className="container-page flex h-16 items-center">
        <span className="whitespace-nowrap text-lg font-bold tracking-tight text-white">Watch<span className="text-[#ff1493]" aria-hidden>VERD<span className="wv-iflip"><span className="wv-iflip-inner"><span className="wv-iflip-face">I</span><span className="wv-iflip-face wv-iflip-back">1</span></span></span>CT</span><span className="sr-only">Verdict</span> · ⚖️ {t('together.court')}</span>
      </header>
