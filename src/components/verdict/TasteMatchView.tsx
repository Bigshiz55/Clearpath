...
function tone(match: number): { text: string; ring: string; bar: string; word: string } {
  if (match >= 75) return { text: 'text-emerald-300', ring: 'ring-emerald-400/40', bar: 'bg-emerald-400', word: 'Strong match' };
  if (match >= 58) return { text: 'text-gold-300', ring: 'ring-gold-400/40', bar: 'bg-gold-400', word: 'Good match' };
  if (match >= 45) return { text: 'text-slate-200', ring: 'ring-white/20', bar: 'bg-slate-300', word: 'Mixed' };
  return { text: 'text-red-300', ring: 'ring-red-400/40', bar: 'bg-red-400', word: 'Against your taste' };
}
...
export function TasteMatchView({ tm }: { tm: TasteMatch }) {
  ...
  return (
    <section className="card p-5 sm:p-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="flex items-center gap-2 text-lg font-bold text-white">
            <span aria-hidden>🧬</span> Your taste match
          </h2>
          <p className="mt-0.5 text-xs text-slate-400">How this title’s DNA fits the dimensions you keep rating highly.</p>
        </div>
      ...
      <div className={`mt-1 text-xs font-semibold ${c.text}`}>{c.word}</div>

      {(tm.agree.length > 0 || tm.clash.length > 0) && (
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {tm.agree.length > 0 && (
            <div className="rounded-xl border border-emerald-400/25 bg-emerald-500/10 p-3">
              <div className="text-xs font-bold uppercase tracking-wide text-emerald-300">Matches you</div>
          ...
          {tm.clash.length > 0 && (
            <div className="rounded-xl border border-red-400/25 bg-red-500/10 p-3">
              <div className="text-xs font-bold uppercase tracking-wide text-red-300">Against your lean</div>
              <ul className="mt-1.5 space-y-1 text-sm text-slate-200">
                {tm.clash.map((a) => (
      ...
      {tm.dials.length > 0 && (
        <div className="mt-4">
          <div className="text-xs font-semibold uppercase tracking-wide text-slate-400">Your taste dials</div>
      ...
      <p className="mt-3 text-[11px] text-slate-500">Learned from {tm.samples} titles you’ve rated. Rate more to sharpen it.</p>
    </section>
  );
}
