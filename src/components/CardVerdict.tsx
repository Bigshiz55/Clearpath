  ...
  const button = (side: Ruling) => {
    ...
    return (
      ...
      >
        ...
        {active ? (
          <svg viewBox="0 0 24 24" className="wv-act-icon h-3.5 w-3.5 flex-none" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
            <path d="M9 14 4 9l5-5" />
            <path d="M4 9h11a5 5 0 0 1 0 10h-3" />
          </svg>
        ) : (
          <svg viewBox="0 0 24 24" className={`wv-act-icon h-3.5 w-3.5 flex-none ${side === 'against' ? '-scale-x-100' : ''}`} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
        ...
        <span className="wv-act-label whitespace-nowrap font-black uppercase tracking-wide">{RULING_LABEL[side]}</span>
      </button>
    );
  };
