...
export function CardDna({ mediaType, tmdbId, className = '' }: { mediaType: MediaType; tmdbId: number; className?: string }) {
  ...
  return (
    <div
      ...
      title={t('title.cardDnaTip', { score: dna.tasteScore })}
    >
      <Verd1ctBadge score={dna.tasteScore} px={38} />
      <span className="flex min-w-0 flex-col leading-tight">
        <span className="text-[10px] font-black uppercase tracking-wide text-pink-100">{t('verdict.yourVerdict')}</span>
        <span className="text-[9px] font-semibold uppercase tracking-wide text-pink-100/70">{t('title.yourTasteMatch')}</span>
      </span>
    </div>
  );
}
