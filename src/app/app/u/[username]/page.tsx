...
export const metadata: Metadata = { title: 'Profile · WatchVerd1ct' };
...
export default async function PublicProfilePage({ params }: { params: { username: string } }) {
  ...
  if (res.kind === 'not_found') {
    return (
      <div className="card mx-auto max-w-lg p-8 text-center">
        ...
        <p className="mt-2 text-sm text-slate-400">There’s no WatchVerd1ct user with that username.</p>
  ...
  const p = res.profile;
  const name = p.displayName?.trim() || (p.username ? `@${p.username}` : 'WatchVerd1ct user');
