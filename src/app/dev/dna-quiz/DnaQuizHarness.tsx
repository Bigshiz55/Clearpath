...
const MOCK: QuizItem[] = [
  ...
  { id: 27205, mediaType: 'movie', title: 'Inception', year: 2010, posterPath: '/i.jpg', posterUrl: 'data:image/gif;base64,R0lGODlhAQABAAAAACw=', genre: 'Sci-Fi' },
  { id: 2316, mediaType: 'tv', title: 'The Office', year: 2005, posterPath: '/o.jpg', posterUrl: 'data:image/gif;base64,R0lGODlhAQABAAAAACw=', genre: 'Comedy' },
];
...
export function DnaQuizHarness() {
  ...
  const onUndo = async (eventId: string, watchlistItem?: { tmdbId: number; mediaType: 'movie' | 'tv' }) => {
    if (typeof window !== 'undefined') {
      (window.__quizUndos ??= []).push(eventId);
      if (watchlistItem) (window.__quizWatchlistUndos ??= []).push(watchlistItem.tmdbId);
    }
    return { ok: true as const };
  };
  return (
