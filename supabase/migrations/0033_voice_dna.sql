-- DEPRECATED — NOT APPLIED, AND MUST NOT BE.
--
-- This created `voice_dna_sessions` for the Taste Interview, which was retired
-- before the migration ever ran against any environment. It is deliberately
-- NOT registered in PENDING_MIGRATIONS, so the table is never created and no
-- data can accumulate in it.
--
-- The file is kept rather than deleted so the history of the decision is
-- legible. If it is ever confirmed that no deployment created this table, the
-- file can go too. Nothing reads or writes it.
--
...
--   -- Quick or full. The interview is adaptive, so this sets the ground it
--   -- covers rather than a fixed question count.
